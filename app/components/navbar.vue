<template>
  ...
  >
    <div class="relative text-sm py-4 uppercase flex items-center gap-8">
      ...
      <NuxtLink
        class="hidden sm:flex text-white/30"
        :class="{ '!text-white': $route.path.startsWith('/interests') }"
        to="/interests"
      >
        Interests
      </NuxtLink>
      ...
      <Icon
        class="inline sm:!hidden text-3xl cursor-pointer"
        name="mdi:hamburger"
        @click="drawer?.open()"
      />
      ...
      <NuxtImg
        src="/img/palestine-flag@3x.svg"
        class="cursor-pointer absolute top-0 right-0 h-full hover:scale-[1.2] origin-top"
